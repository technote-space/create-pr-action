...
const npmInstallHint = async (pkgs, analysis, options) => {
    ...
    const installHint = `${chalk.cyan(packageManager + ' install')}${pkgs.length > 1 ? ' in each project directory' : ''} to install new versions`;
...
async function run(runOptions = {}, { cli } = {}) {
    ...
    if (options.timeout) {
        const timeoutMs = lodash_1.default.isString(options.timeout) ? Number.parseInt(options.timeout, 10) : options.timeout;
