...
const { sleep } = github_action_helper_1.Utils;
const { isPr, isCron } = github_action_helper_1.ContextHelper;
...
const createPr = (logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!misc_1.isTargetBranch(misc_1.getPrHeadRef(context))) {
        return;
    }
    if (isCron(context)) {
    ...
    if (!files.length) {
        ...
        if (!(yield command_1.getRefDiff(misc_1.getPrHeadRef(context), logger)).length) {
        ...
        // Commit local diffs
        yield command_1.commit(logger);
        if (!(yield command_1.getRefDiff(misc_1.getPrHeadRef(context), logger)).length) {
            // Close if there is no diff
            yield command_1.getApiHelper(logger).closePR(branchName, octokit, context);
            return;
        }
        yield command_1.push(branchName, logger, context);
    }
    if (files.length) {
    ...
    if (!mergeable) {
        ...
        yield command_1.resolveConflicts(branchName, logger, octokit, context);
    }
    if (isCron(context)) {
        // Sleep
